...
DEFAULT_STATE = {
    ...
    "agent_pending_answer": None,  # NEW: Store draft answer
}
...
def restart_agent(user_question, filtered_df, show_chart=False):
    ...
    set_state("agent_pending_answer", None)  # NEW: Reset pending answer
...
def run_step(client):
    ...
    if phase == "thinking":
        ...
        if reasoning.use_tool:
            ...
            # MODIFIED: Instead of going directly to "done", go to "awaiting_answer_confirmation"
            get_state("agent_events").append({"type": "thought", "thought": reasoning.reason})
            set_state("agent_pending_answer", {
                "thought": reasoning.reason,
                "answer": reasoning.answer
            })
            set_state("agent_phase", "awaiting_answer_confirmation")
...
# NEW: Accept the final answer
def accept_pending_answer():
    pending_answer = get_state("agent_pending_answer")
    get_state("agent_events").append({
        "type": "answer",
        "thought": pending_answer["thought"],
        "answer": pending_answer["answer"]
    })
    set_state("agent_pending_answer", None)
    set_state("agent_phase", "done")

# NEW: Request revision of the answer
def revise_pending_answer(revision_feedback):
    messages = get_state("agent_messages")
    pending_answer = get_state("agent_pending_answer")
    
    # Add user feedback to conversation
    feedback_msg = f"User wants a revision. Feedback: {revision_feedback if revision_feedback else 'Please provide a better or more detailed answer.'}"
    messages.append({"role": "user", "content": feedback_msg})
    
    # Record the revision request in events
    get_state("agent_events").append({
        "type": "revision_requested",
        "answer_draft": pending_answer["answer"],
...
def render_events():
    for event in get_state("agent_events"):
        if event["type"] == "thought":
            ...
            st.divider()
        elif event["type"] == "revision_requested":  # NEW
            st.markdown(f"**🔄 Answer Revision Requested**")
            with st.expander("View draft answer"):
                st.write(event['answer_draft'])
            if event.get("feedback"):
                st.text(f"Feedback: {event['feedback']}")
            st.divider()
...
def render_pending_approval():
    st.warning("⚠️ The agent wants to perform the following action:")
...
# NEW: Render answer confirmation UI
def render_pending_answer_confirmation():
    pending_answer = get_state("agent_pending_answer")
    
    st.success("✅ The agent has formulated a final answer:")
    
    # Show the draft answer in a nice box
    with st.container():
        st.markdown("**📝 Draft Answer:**")
        st.info(pending_answer["answer"])
    
    st.markdown("**What would you like to do?**")
    
    btn_col1, btn_col2 = st.columns(2)
    with btn_col1:
        accepted = st.button("✅ Accept Answer", type="primary", use_container_width=True, help="Accept this answer as the final result")
    with btn_col2:
        request_revision = st.button("🔄 Request Revision", use_container_width=True, help="Ask the agent to revise or improve the answer")
    
    return accepted, request_revision

def render_revision_feedback():
    st.info("💡 Tell the agent how to improve the answer:")
    feedback = st.text_area(
        "Revision instructions:",
        key="revision_feedback",
        placeholder="e.g., 'Provide more specific examples', 'Include statistical details', 'Make it more concise'",
        height=100
    )
    submitted = st.button("Submit Revision Request", type="primary", use_container_width=True)
    return submitted, feedback
...
def render_panel():
    ...
    with container:
        ...
        elif phase == "awaiting_answer_confirmation":  # NEW
            with st.expander("🧠 Agent Reasoning Trace", expanded=True):
                render_events()
            accepted, request_revision = render_pending_answer_confirmation()
            actions = {"accepted": accepted, "request_revision": request_revision}

        elif phase == "awaiting_revision_feedback":  # NEW
            with st.expander("🧠 Agent Reasoning Trace", expanded=True):
                render_events()
            submitted, feedback = render_revision_feedback()
            actions = {"submitted": submitted, "revision_feedback": feedback}
...
def agent_panel(client, analyze_button, user_question, filtered_df, show_chart=False):
    # MODIFIED Phases: 
    # idle -> thinking <-> acting -> awaiting_approval -> thinking ... 
    #                              -> awaiting_answer_confirmation -> awaiting_revision_feedback -> thinking ...
    #                              -> done
    ...
    if phase in ("thinking", "acting"):
        ...
        if actions.get("accepted"):
            accept_pending_answer()
            st.rerun()
        elif actions.get("request_revision"):
            set_state("agent_phase", "awaiting_revision_feedback")
            st.rerun()
    elif phase == "awaiting_revision_feedback" and actions.get("submitted"):  # NEW
        revise_pending_answer(actions.get("revision_feedback", ""))
        st.rerun()
# ── State ──
